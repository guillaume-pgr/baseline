...
- RÈGLE STRICTE : chaque valeur d'antériorité est rattachée à UNE SEULE date = l'en-tête de date le PLUS PROCHE AU-DESSUS de cette ligne, valable jusqu'au prochain en-tête. N'attribue JAMAIS la même valeur à plusieurs dates. Le nombre d'entrées de prior_values d'un marqueur = le nombre de colonnes de valeur réellement imprimées pour CE marqueur (souvent 1, parfois 0).
- Un en-tête de date propre à une SECTION (ex. 23/08/2024 au-dessus de l'HbA1c) ne s'applique QU'AUX marqueurs de cette section. Un marqueur d'une autre section (ex. cholestérol) suit l'en-tête qui le précède (ex. 25/04/2025 réapparaissant avant les lipides) — ne lui recopie PAS la date d'une autre section.
- Il peut y avoir PLUSIEURS dates différentes dans le même document (ex. la plupart des marqueurs comparés au 25/04/2025, mais l'HbA1c au 23/08/2024) — n'en suppose donc PAS une seule globale, et n'applique pas non plus toutes les dates à tous les marqueurs.
